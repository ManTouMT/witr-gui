...
## ⚡ 极速即用：终端一行命令，零安装秒级启动（极客推荐）

不想下载客户端？只要你的系统安装了 Node.js，在任何终端直接输入：

```bash
npx witr-gui
```

> 💡 **0 安装、0 磁盘占用**：执行后将在 1 秒内自动调起默认浏览器，呈现与桌面端 100% 一致的 **React Flow 交互式矢量拓扑图** 与 **双向因果家族树**，随时按 `Ctrl + C` 退出，用完即走！
