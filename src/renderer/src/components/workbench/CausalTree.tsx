...
export const CausalTree: React.FC = () => {
  ...
  const getNodeRole = (index: number, total: number, node: ProcessInfo) => {
    if (index === total - 1) return { label: '目标进程 (Target Process)', color: 'border-blue-500 bg-blue-950/40 text-blue-300' }
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between pb-3 border-b border-neutral-800/80">
        <div>
          ...
          <p className="text-xs text-neutral-400 mt-0.5">
            自顶向下追踪进程如何被启动（Who started whom and why）
          </p>
        </div>
      ...
      {witrResult?.Warnings && witrResult.Warnings.length > 0 && (
        <div className="p-3.5 rounded-xl bg-amber-950/30 border border-amber-800/50 text-amber-200 text-xs space-y-1">
          <div className="flex items-center gap-1.5 font-semibold text-amber-300">
            <AlertTriangle className="w-4 h-4" />
            <span>安全审计与异常提示</span>
          </div>
          <ul className="list-disc list-inside space-y-0.5 text-amber-200/90 pl-1 font-mono text-[11px]">
            {witrResult.Warnings.map((w, idx) => (
              <li key={idx}>{w}</li>
            ))}
          </ul>
        </div>
      ...
      <div className="relative pl-6 space-y-6 before:absolute before:left-3 before:top-4 before:bottom-4 before:w-0.5 before:bg-gradient-to-b before:from-purple-500 before:via-blue-500 before:to-emerald-500">
            ...
            <div key={`${node.PID}-${index}`} className="relative group">
              ...
              <div
                className={`absolute -left-6 top-4 w-3.5 h-3.5 rounded-full border-2 transform -translate-x-1/2 flex items-center justify-center transition ${
                  isTarget
                    ? 'bg-blue-500 border-blue-200 shadow-lg shadow-blue-500/50 ring-4 ring-blue-500/20'
                ...
                <div className="flex items-center justify-between mb-2.5">
                  ...
                  <div className="flex items-center gap-2 text-xs font-mono">
                    {node.MemoryRSS && node.MemoryRSS > 0 ? (
                      <span className="flex items-center gap-1 text-neutral-400 px-1.5 py-0.5 rounded bg-neutral-800/60 border border-neutral-700/40">
                ...
                <div className="mt-2.5 pt-2 border-t border-neutral-800/60 grid grid-cols-2 gap-2 text-xs text-neutral-400">
                    ...
                    <div className="col-span-2 flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <button
                          ...
                          className="px-1.5 py-0.5 rounded bg-blue-950/60 hover:bg-blue-900/60 text-blue-300 text-[10px] font-medium border border-blue-800/40"
                        >
                          VS Code
                        </button>
                        <button
                          onClick={() => openPath(node.WorkingDir!, 'cursor')}
                          className="px-1.5 py-0.5 rounded bg-purple-950/60 hover:bg-purple-900/60 text-purple-300 text-[10px] font-medium border border-purple-800/40"
                        >
                          Cursor
                        </button>
                        <button
                  ...
                  {node.GitBranch && (
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-emerald-400 font-mono text-[11px]">{node.GitBranch}</span>
                    </div>
                  )}
                  ...
                  {node.Container && (
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-cyan-300 font-mono text-[11px]">{node.Container}</span>
                    </div>
                  )}
